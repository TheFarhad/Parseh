...
public static partial class Extention
{
    ...
    public static bool Is<T>(this object source, out T output)
    {
        ...
        output = result ? source.As<T>() : default!;
        return result;
    }
    ...
    public static T To<T>(this object source)
    {
        ...
        result = Convert.ChangeType(source, typeCode).As<T>();
        return result;
    }

    public static int ToInt(this object source) => source.To<int>();
    public static ulong ToULong(this object source) => source.To<ulong>();
    public static long ToLong(this object source) => source.To<long>();
    ...
    public static T GetAttribute<T>(this object source) where T : Attribute
        => source.GetType().GetAttribute<T>();

    public static PropertyInfo? GetPropertyInfo(this object source, string property)
        => source
            .GetType()
            .GetProperty(property, BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance);

    public static Type? GetPropertyType(this object source, string property)
        => source.GetPropertyInfo(property)?.PropertyType;

    public static void Set(this object source, string property, object value)
        => source.GetPropertyInfo(property)?.SetValue(source, value);

    public static object? Get(this object source, string property)
        => source.GetPropertyInfo(property)?.GetValue(source);

    public static T Get<T>(this object source, string property)
    {
        var value = source.Get(property);
        T result = value is not null ? value.As<T>() : default!;
        return result;
    }

    public static bool IsNullable(this Type type)
        => !Nullable.GetUnderlyingType(type)!.IsNull() || !type.IsValueType;

    public static Type FirstGenericType(this Type type) => type.GenericTypeArguments.First();

    public static object? Generic(this Type type, params Type[] genericTypes)
        => type.MakeGenericType(genericTypes).Create();

    public static T? Generic<T>(this Type type, params Type[] genericTypes)
        => type.Generic(genericTypes)!.As<T>();
    ...
    public static T GetAttribute<T>(this PropertyInfo info) where T : Attribute
       => info.GetCustomAttributes(typeof(T), true).FirstOrDefault()!.As<T>();

    public static bool ShoudSet(this PropertyInfo info) => info.CanWrite;
...
public static partial class Extention
{
    ...
    public static string Format(this string source, params string[] parameters)
    {
        var result = source;
    ...
    public static bool IsGuid(this string source, out Guid output) => Guid.TryParse(source, out output);

    public static byte[] ToBytes(this string source) => Encoding.UTF8.GetBytes(source);
    public static string TotString(this byte[] source) => Encoding.UTF8.GetString(source);
    public static byte[] ToBytes(this Stream source) => source.As<MemoryStream>().ToArray();
    public static string AsString(this byte[] source) => Encoding.UTF8.GetString(source);

    public static string ApplyCorrectYeKe(this object source)
        => source.IsNull() ? "" : source.ToString().ApplyCorrectYeKe();
...
public static partial class Extention
{
    public static async Task<TOutput?> GetAsJsonAsync<TValue, TOutput>(this HttpClient httpClient, string requestUri, TValue value, UriKind uriKind = UriKind.Absolute, CancellationToken token = default)
     => await SendAsJsonAsync<TValue, TOutput>(httpClient, requestUri, HttpMethod.Get, value, uriKind, token);

    public static async Task<TOutput?> DeleteAsJsonAsync<TValue, TOutput>(this HttpClient httpClient, string requestUri, TValue value, UriKind uriKind = UriKind.Absolute, CancellationToken token = default)
    => await SendAsJsonAsync<TValue, TOutput>(httpClient, requestUri, HttpMethod.Delete, value, uriKind, token);

    private static async Task<TOutput?> SendAsJsonAsync<TSource, TOutput>(this HttpClient httpClient, string requestUri, HttpMethod method, TSource source, UriKind uriKind = UriKind.Absolute, CancellationToken token = default)
    {
        TOutput? result = default!;
        var response = await SendAsJsonAsync(httpClient, requestUri, method, source, uriKind, token);
    ...
    private static async Task<HttpResponseMessage> SendAsJsonAsync<TSource>(this HttpClient httpClient, string requestUri, HttpMethod method, TSource source, UriKind uriKind = UriKind.Absolute, CancellationToken token = default)
    {
        var request = new HttpRequestMessage
        {
            Content = JsonContent.Create(source),
            Method = method,
...
public static partial class Extention
{
    ...
    public static Responser RequestPipeService(this HttpContext source)
        => source.Service<Responser>();
}
#endregion
...
public static partial class Extension
{
    ...
    public static T? Create<T>(this Type type, params object[] args) where T : class, new()
        => type.Create(args)?.As<T>();
}
